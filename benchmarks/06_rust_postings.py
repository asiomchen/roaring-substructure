...
def run_worker(
    ...
    index_class = (
        NumpySubstructureQueryIndex if backend == "numpy" else RustSubstructureQueryIndex
    )
    ...
    if backend == "numpy":
        ...
        index_bytes = len(pickle.dumps(index, protocol=5))
    else:
        bitmap_bytes = index.postings.nbytes()
...
def main() -> None:
    ...
    parser.add_argument(
        "--backend", choices=("both", "numpy", "rust"), default="both"
    )
    parser.add_argument(
    ...
    backends = ("numpy", "rust") if args.backend == "both" else (args.backend,)
    results = []
    ...
    if len(results) == 2 and (results[0]["matches"], results[0]["digest"]) != (
        ...
        raise RuntimeError("NumPy and Rust indexes returned different matches")
